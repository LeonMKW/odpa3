...
def space_weather_report_content(tf1, tf2, get_F10point7, get_ApIndex, get_KpIndex, satID_list,
    ...
    if F107_today != "暂未更新":
        if float(F107_today) < 70:
            ...
            past12hours += "极高"
        past12hours += f" (F10.7值={F107_today})"
    else:
        past12hours += "暂未更新"
    ...
    if Kp_value_max != "地磁暴活动暂未更新":
        if float(Kp_value_max) <= 1:
            past12hours += "地磁暴活动强度整体平静"
        elif 2 < float(Kp_value_max) <= 3:
            past12hours += "地磁暴活动强度整体稍有扰动"
        elif 3 < float(Kp_value_max) <= 4:
            past12hours += "地磁暴活动强度整体活跃"
            ...
            past12hours += "达到极端地磁暴级别"
        past12hours += f" (Kp指数={Kp_value_max})"
    else:
        past12hours += "地磁暴活动暂未更新"
    ...
    if Ap_today != "暂未更新":
        if float(Ap_today) <= 7:
            ...
            past12hours += "达到整体小地磁暴级别"
        elif 50 < float(Ap_today) <= 100:
            past12hours += "达到整体大地磁暴级别"
        else:
            past12hours += "达到整体极端地磁暴级别"
        past12hours += f" (Ap指数={Ap_today})"
    else:
        past12hours += "暂未更新"

    # **Generate `future12hours` String**
    future12hours = f"太阳活动水平 "
    if F107_tomorrow != "暂无预报":
        if float(F107_tomorrow) < 70:
            ...
            future12hours += "极高"
        future12hours += f" (F10.7值={F107_tomorrow})"
    else:
        future12hours += "暂无预报"
    ...
    if Ap_tomorrow != "暂无预报":
        if float(Ap_tomorrow) <= 7:
            ...
            future12hours += "整体轻度扰动"
        elif 16 < float(Ap_tomorrow) <= 30:
            future12hours += "整体活跃"
        elif 30 < float(Ap_tomorrow) <= 50:
            future12hours += "可能达到整体小地磁暴级别"
        elif 50 < float(Ap_tomorrow) <= 100:
            future12hours += "可能达到整体大地磁暴级别"
        else:
            future12hours += "可能达到整体极端地磁暴级别"
        future12hours += f" (Ap指数={Ap_tomorrow})"
    else:
        future12hours += "暂无预报"
    ...
    advisory_f107 = ""
    if F107_tomorrow != "暂无预报":
        if float(F107_tomorrow) >= 200:
            advisory_f107 = "未来太阳活动高，需要关注卫星OBC复位和切机情况。需要注意文件巡检结果。"
        elif 170 <= float(F107_tomorrow) < 200:
            advisory_f107 = "未来太阳活动较高，请关注卫星OBC复位和切机情况。请注意文件巡检结果。"
        elif 150 <= float(F107_tomorrow) < 170:
            advisory_f107 = "未来太阳活动中等，建议关注卫星OBC复位和切机情况。"
        else:
            advisory_f107 = "未来太阳活动较低，对卫星影响较小。"

    # **ApIndex Advisory**
    advisory_ap = ""
    if Ap_tomorrow != "暂无预报":
        if float(Ap_tomorrow) > 30:
            advisory_ap = "地磁活动达到地磁暴级别，需要关注卫星轨道高度变化。"
        elif 20 < float(Ap_tomorrow) <= 30:
            advisory_ap = "地磁活动有较强扰动，建议关注卫星轨道高度变化。"
        elif 15 < float(Ap_tomorrow) <= 20:
            advisory_ap = "地磁活动有中度扰动，建议关注卫星轨道高度变化。"
        elif 10 < float(Ap_tomorrow) <= 15:
            advisory_ap = "地磁活动有轻度扰动，建议关注卫星轨道高度变化。"
        elif float(Ap_tomorrow) < 10:
            advisory_ap = "地磁活动较低，对卫星影响较小。"

    # **Combine the Two Advisory Messages**
    advisory = advisory_f107 + " " + advisory_ap if advisory_f107 or advisory_ap else "未来空间环境整体平静。"
    ...
    extracted_satellite_data = [
        {
            ...
            "code": satellite_code_map.get(sat["spacecraftId"], "未知代码"),  # Add code mapping
            "epochTimeUTC": sat["epochTimeUTC"],
            "mse": sat["mse"],
            "ephemeris_error": sat["ephemeris_error"],
            "difference": sat["difference"],
            "mean_altitude": altitude_data.get(sat["spacecraftId"], "暂无数据"),
            "altitude_change": altitude_change_data.get(sat["spacecraftId"], "暂无数据"),
        }
